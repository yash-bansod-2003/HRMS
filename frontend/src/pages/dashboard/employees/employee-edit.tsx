...
const EmployeeEditPage = () => {
  ...
  const query = useQuery({
    queryKey: ["auth/employees/", employeeId],
    enabled: !!employeeId,
    queryFn: async () => {
      const response = await httpClient.get(`/api/employees/${employeeId}/?format=json`);
  ...
  const mutation = useMutation({
    mutationFn: async (data: z.infer<typeof formSchema>) => {
      if (!employeeId) {
        throw new Error("Employee id is missing from route params");
      }

      const response = await httpClient.put(`/api/employees/${employeeId}/`, data);
      return response.data;
    },
  ...
  if (!employeeId || (!query.isLoading && !query.data) || mutation.isSuccess || query.isError) {
    ...
    <div className="px-4 lg:px-6">
      ...
      >
        <DashboardHeader
          ...
          text="Update the employee's information."
        >
          <Button type="submit" disabled={mutation.isPending}>
            <PlusCircle className="mr-2" /> Update Employee
          </Button>
        </DashboardHeader>
